...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const handleProfileUpdated = (event) => {
      ...
      setProfile((current) => current
        ? { ...current, user_metadata: { ...current.user_metadata, ...updates } }
        : current);
    };
  ...
  const businessCategory = businessPageData.registrationCategory
    || profile?.user_metadata?.business_domain
    || 'Business workspace';
  ...
  const renderActiveSection = () => {
    ...
    if (activeItem === 'Appointment Form') {
      return <AppointmentFormBuilder />;
    }

    if (activeItem === 'Configure Bookings') {
      return <BookingTypes />;
    }

    if (activeItem === 'My Business Page') {
      return <BusinessPageManager initialData={businessPageData} />;
    }

    if (activeItem === 'Analytics and Reports') {
      return <AnalyticsReports />;
    }

    if (activeItem === 'Team Members') {
        ...
        <header className="dashboard-header">
          <div>
            <span className="dashboard-eyebrow">
              {member ? 'Team member workspace' : businessCategory}
            </span>
            <h1>Hello, {member ? member.full_name : businessName}</h1>
            <p>
              {member
                ? 'View appointments, download reports, and update patient statuses.'
                : `Here is what is happening across your healthcare workspace today.${customerId ? ` Customer ID: ${customerId}` : ''}`}
            </p>
          </div>
