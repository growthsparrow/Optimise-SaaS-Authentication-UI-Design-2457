...
function SignIn() {
  ...
  const handleOwnerSubmit=async (event)=> {
    ...
    try {
      const enabled=await isUserEnabled();

      if (!enabled) {
        await supabase.auth.signOut();
        setNotice('Your account has been disabled. Please contact the administrator.');
        return;
      }
        ...
        <form className="auth-form" onSubmit={handleMemberSubmit}>
          <label className="field" htmlFor="member-id">
            ...
            <span className="input-wrap">
              <SafeIcon icon={FiUser} className="input-icon" aria-hidden="true" />
              <input
                id="member-id"
                name="member-id"
                placeholder="OP2345"
          ...
          <label className="field" htmlFor="member-password">
            ...
            <span className="input-wrap">
              <SafeIcon icon={FiKey} className="input-icon" aria-hidden="true" />
              <input
                id="member-password"
