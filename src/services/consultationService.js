...
export async function saveConsultation(values, existingConsultation) {
  const payload = {
  ...
  const query = existingConsultation
    ? supabase.from(consultationsTable).update(payload).eq('id', existingConsultation.id).select().single()
    : supabase.from(consultationsTable).insert(payload).select().single();
...
export async function setConsultationBookingStatus(consultationId, isAvailableForBooking) {
  const {data, error} = await supabase
    .from(consultationsTable)
...
export async function listConsultationsForDoctor(doctorId) {
  const {data, error} = await supabase
    .from(consultationsTable)
    .select('*')
    .eq('is_archived', false)
